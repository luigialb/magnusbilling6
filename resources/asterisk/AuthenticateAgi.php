...
class AuthenticateAgi
{
    ...
    public static function techPrefixAuthenticate(&$MAGNUS, &$agi, $authentication)
    {
        ...
        $sql       = "SELECT *, u.id id, u.id_user id_user FROM pkg_user u INNER JOIN pkg_plan p ON u.id_plan = p.id WHERE u.techprefix = '$tech'  LIMIT 1";
        $modelUser = $agi->query($sql)->fetch(PDO::FETCH_OBJ);

        if ($authentication != true && isset($modelUser->id)) {
